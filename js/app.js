...
todomvc_App.main = function() {
	var todo = new todomvc_App("todomvc-haxe");
	todo.render();
	Router({'/:filter': function (filter) { todo.filter = filter; todo.render(); } }).init('/all');
};
todomvc_App.prototype = {
	cacheElements: function() {
		...
		this._newTodo = this._header.find("#new-todo");
		this._toggleAll = this._main.find("#toggle-all");
		this._todoList = this._main.find("#todo-list");
		this._count = this._footer.find("#todo-count");
		this._clearBtn = this._footer.find("#clear-completed");
	}
	,bindEvents: function() {
		this._newTodo.on("keyup",null,$bind(this,this.onNewTodoKeyUp));
		this._toggleAll.on("change",null,$bind(this,this.onToggleAllChange));
		this._footer.on("click","#clear-completed",$bind(this,this.onClearClick));
		this._todoList.on("change",".toggle",$bind(this,this.onToggleChange));
		this._todoList.on("dblclick","label",$bind(this,this.onLabelDoubleClick));
		this._todoList.on("keyup",".edit",$bind(this,this.onEditKeyUp));
		this._todoList.on("focusout",".edit",$bind(this,this.onEditBlur));
	...
	,onClearClick: function(evt) {
		if(this.store.overwrite(this.getActiveTodos())) this.todos = this.store.findAll();
		this.filter = "all";
		this.render();
	}
	...
	,onEditBlur: function(evt) {
		...
		if(val != "") this.todos[i].title = val; else this.todos.splice(i,1);
		this.store.update(this.todos[i]);
		this.render();
	}
	,onDestroyClick: function(evt) {
		var el = evt.target;
		var i = this.indexFromEl(el);
		if(this.store.remove(this.todos[i].id)) this.todos.splice(i,1);
		this.render();
	}
	...
	,renderFooter: function() {
		...
		if(todoCount > 0) {
			var template = this.footerTemplate.execute({ activeTodoCount : activeTodoCount, activeTodoWord : activeTodoCount != 1?"items":"item", completedTodos : todoCount - activeTodoCount, filter : this.filter});
			this._footer.html(template);
			this._footer.show();
		} else this._footer.hide();
	}
	,render: function() {
		var filteredTodos = this.getFilteredTodos();
		var formatTodo = function(todo) {
			return { checked : todo.completed?"checked":"", completed : todo.completed?"completed":"", id : todo.id, title : todo.title};
		};
		this._todoList.html(((function($this) {
			var $r;
			var _g1 = [];
			{
				var _g2 = 0;
				while(_g2 < filteredTodos.length) {
					var todo1 = filteredTodos[_g2];
					++_g2;
					_g1.push($this.todoTemplate.execute(formatTodo(todo1)));
				}
			}
			$r = _g1;
			return $r;
		}(this))).join(""));
		var activeTodos = this.todos.length > 0;
		if(activeTodos) this._main.show(); else this._main.hide();
		this._toggleAll.prop("checked",activeTodos);
		this.renderFooter();
		this._newTodo.focus();
	}
...
var todomvc_Store = function(prefix,storage) {
	this.prefix = prefix;
